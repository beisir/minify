...
function _getBasePath() {
	...
	for (var i = 0, len = els.length; i < len; i++) {
		...
		if(/hc\.kindeditor\.min\.js/.test(src)){
...
function _create(expr, options) {
	...
	_loadScript(editor.langPath + editor.langType + '.js', function() {
		create(editor);
	});
	return editor;
}
...
KindEditor.lang({
	...
	'fontname.fontName' : {
		...
		'SimHei' : '黑体',
		// 'Microsoft YaHei' : '微软雅黑', // 2018-03-02 干掉
		'Arial' : 'Arial',
		...
		edit : function() {
			var lang = self.lang(name + '.'),
				html = '<div style="padding:20px;">' +
					...
					'</div>' +
					'<div style="color:red">请输入*.hc360.com域下的链接</div></div>',
				dialog = self.createDialog({
					name : name,
					width : 450,
					title : self.lang(name),
					...
					yesBtn : {
						...
						click : function(e) {
							var url = K.trim(urlBox.val());
							if (url == '//' || K.invalidUrl(url)||(!/([A-Za-z0-9-_]+)\.hc360\.com/g.test(url))) {
								alert(self.lang('invalidUrl'));
								urlBox[0].focus();
								return;
							}
